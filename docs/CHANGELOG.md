...
## 2026-09-12（P03/P05，开发中）

- **固定制品部署与回执**：新增本机/SSH部署器，校验制品与公开配置，拉取固定digest并实际核对就绪；原子成功回执/状态、部署锁与快照回退，保留已有秘密文件。替换旧PowerShell覆盖env的流程，生产数据库配置改为必填引用并补集成变量。首版已在独立Docker项目实际部署，第二版升级/回退验收继续执行；操作说明见 docs/features/deployment-receipts.md。

- **独立生产镜像与制品基础**：17镜像携带生产依赖、迁移及运行资源；后端迁移失败即退出、数据库断连返回503并支持恢复，Gateway聚合就绪。新增全平台容器冷启动与持久化回归，CI smoke接入真实镜像；main发布改为源码SHA和registry digest清单。P03本地独立验收通过，P05部署回执与回退继续实施；完整记录见 docs/validation/2026-09-12-runtime-images.md。
