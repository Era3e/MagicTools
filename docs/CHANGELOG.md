...
- **独立生产镜像与制品基础**：17镜像携带生产依赖、迁移及运行资源；后端迁移失败即退出、数据库断连返回503并支持恢复，Gateway聚合就绪。新增全平台容器冷启动与持久化回归，CI smoke接入真实镜像；main发布改为源码SHA和registry digest清单。P03/P05本地独立验收通过，CI分别保存候选质量、镜像运行及同制品配置升级回归的证据；完整记录见 docs/validation/2026-09-12-runtime-images.md。
