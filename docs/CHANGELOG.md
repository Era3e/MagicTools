...
## 2026-09-16（P27 Code Wiki 与依赖图加固）

- **知识体系三层防线**：修复 Code Wiki 相对链接与陈旧事实，新增 Markdown 链接存在性守卫；引入 dependency-cruiser 扫描 apps/packages，禁止循环依赖、公共包反向依赖应用、应用间直接源码引用和生产代码引用测试，并拆除 4 个既有循环。`graph:json` 生成模块级依赖索引，完整图仅保留在 .qa 产物；CI 质量运行时升级 Node 22。
...
- **自动执行交付可视化与通知闭环**：执行成功回写必须携带全绿结构化验收、candidate SHA、PR 与证据摘要，并与 job/run 租约身份一致；同一事务把需求推进到待验收、保存 PR/部署初始状态并写入稳定 ID 的 outbox 通知。终态失败和取消会通知，中间 retry 不打扰。详情页展示每次 run 心跳、失败原因、独立验收和证据摘要，PR 合并与部署回执分别记录，部署成功前置 PR merged，人工验收不被自动化替代。配置 HTTPS webhook 后通知按租约投递并去重，未配置时明确 not-configured。

## 2026-09-16（P23 隔离编码执行器）

- **自动执行候选与独立验收编排**：新增独立 executor CLI，领取 P22 任务时携带只读需求上下文；编码/验收子进程使用环境白名单、隔离 HOME 与空 Git 配置，拿不到 Manager、GitHub、模型和部署秘密。候选改动先做路径/符号链接/大小边界检查并提交 SHA，再从该 SHA 克隆独立验收工作区执行契约命令；超时终止进程树并保留输出哈希与日志。验收通过后推送固定内容修订分支、复用或创建 PR，并把证据包摘要回写 Manager。真实生产 Codex/GitHub 运行未执行，保持 not-run；通知与自动合并留给 P24/P25。

## 2026-09-16（P22 自动执行任务租约）

- **需求自动执行可靠性基础**：Manager 新增 execution job/run 持久模型，同需求同内容修订唯一，owner 显式排队后才能进入 `owner-token` 自动执行策略。执行器用独立 token 领取任务并获得一次性 run token；心跳、成功、失败均校验租约，过期 run 会被显式恢复并按契约尝试上限 retry/failed，排队后的内容或批准变化会在领取前隔离。本批不启动真实编码进程，P23 前 live executor 保持 not-run。

## 2026-09-16（P21 模型调用观测）

- **模型请求控制与成本证据链**：@mt/model-client 统一支持外部取消、默认/显式超时、重试逐次记录和 AsyncLocalStorage 调用上下文；流式请求要求供应商 usage，缺失时 token 记 NULL 且来源为 unknown，不再用片段数估算。7 个实际调用模型的服务库新增一致 `model_calls` 表，wrapper 异步持久化真实模型身份、状态、重试、耗时、trace/task 和错误，持久化失败不影响业务调用。live provider 本批未真实请求，模型效果与账单核对保持 not-run。

## 2026-09-15（P20 用户帮助与代码检索入口）

- **40 条版本化知识与双入口**：Scholar 固定 20 个用户任务和 20 个开发问题，按 stableId 幂等导入并绑定来源修订、GitHub 证据、需求与验收口径。前台用户帮助目录和公共混合检索只读当前发布快照；后台代码检索仅限 development 空间并沿用管理员鉴权。product 发布必须显式提供版本与部署标识，未知历史动机保留“历史原因：未知”。

## 2026-09-15（P19 统一 Scholar 检索）

- **不可变证据分块与引用对齐**：Scholar 为每个修订生成分块向量与字符区间，公共混合检索固定当前发布产品版本，FTS 与向量候选合并排序并过滤无关近邻。Assistant 移除 Scholar 数据库直连，改经 Gateway 调公共 API；模型只输出候选编号，服务端把真实候选映射为 revision/version/chunk/evidence 引用，虚构编号不能生成引用。

## 2026-09-15（P18 知识空间与版本证据）

- **开发/产品知识隔离与不可变发布**：Scholar 新增知识空间、成员授权、条目不可变修订、产品版本、部署标识、发布记录和 Manager 需求链接。公共列表/详情/FTS/向量 API 在 SQL 层固定 product/public/published/current version；发布后编辑只生成新修订，不改变线上内容与召回向量；撤权、下架和删除会清理可见授权、发布记录、图谱关联与向量索引。

## 2026-09-15（P17 Badcase 闭环）

- **用户澄清、评测失败与需求回归证据链**：低置信度澄清只写建议意图，管理员确认后才进入训练语料；chat/feedback/intent 关联 trace 与消息证据。用户反馈、澄清和 dev/regression 评测失败可转为 badcase，生成 regression 样本并强制先跑修复前 baseline；Manager 以 assistant_badcase 来源幂等创建需求，关闭前校验修复 PR、需求状态、数据集指纹、目标样本与异常计数。
...
- **E2E 稳定业务数据与可访问性**：视觉基线从 18 张扩展到 20 张，新增 Manager 详情档案与 Assistant 长答案会话；Manager 看板和 Scholar 帮助目录撤掉核心区整块遮罩，每张样板页截图前强制断言固定业务文案。样板数据仅由 Playwright 通过公开 API 幂等创建，不改变业务默认数据。Assistant 支持 `?conversation=` 装载历史会话，会话条目可键盘激活；桩模式知识回答保留长上下文，明确用于可复现截图而非真实模型评测。

## 2026-09-15（P13 自动推送与调度回执）

- **Gatherer 自动推送与可观测性**：采集入库真正收集新增条目 ID，`options.autoPush` 开启后仅推送新条目；推送事件改用 item ID 派生的稳定幂等键，已推送条目返回 skipped 且不追加 outbox 事件。运行完成将 run 置为 success，API 新建/更新源后刷新 cron 注册；调度状态展示实际注册、最近运行计数与错误，死信事件在管理入口可追踪。信息源表单增加自动推送开关，后台新增「调度实况」与死信列表。

## 2026-09-14（P11 GitHub 同步与 Webhook）

- **GitHub 全量同步与持久回调**：Issues 按每页100条最多20页分页同步，排除 PR 且超过2000条显式失败；既有 Issue 的标题、正文、标签和状态变化带并发版本写回，冲突不覆盖本地内容。Webhook 移除进程内存去重，新增 delivery 持久租约、崩溃与异常重试恢复、payload 摘要审计；除显式桩模式外强制 HMAC、必须使用原始请求体且生产禁止桩绕过，回执仅允许当前锁持有者更新，PR 事件按 `updated_at` 串行化，乱序回调返回 `out_of_order`。

## 2026-09-14（P12 Outbox 可靠性）

- **Outbox 租约与批处理**：跨库事件领取改为短事务原子写入 `processing` 租约，handler 执行期间并发消费者不会重复执行；进程崩溃后过期租约可恢复，迟到旧消费者不能回写状态。新增 `processOutboxBatch`，Assessor/Manager/Scholar 在业务入库完成后再确认 done，失败整批 retry 并释放租约；重复事件 ID 仍由数据库幂等键去重。
- **可靠性补尾**：事件领取时原子递增尝试次数，消费者进程崩溃也会保留计数，过期且达到上限的 `processing` 行先进入 `dead` 不再执行业务。Assessor 使用稳定 `source_key` 并将请求头与明细同事务写入，重放追加缺失事件和明细；Manager 为 Assessor 来源建立部分唯一索引，并发同来源事件只保留一条需求。唯一索引迁移前显式检测存量重复并要求人工确认，不自动合并业务归属。

## 2026-09-14（P10 仓库证据反向整理）

- **按提交反推代码能力证据**：Assessor 新增仓库证据任务，按提交读取完整变更清单并筛选 routes/controller/service/schema/tests；清单截断或超过200文件显式失败。每个候选绑定提交SHA、源码路径、首末变更行、GitHub链接、变更片段和文件内容哈希，无法推断的业务动机固定标记为 `unknown`；删除文件读取父提交内容并指向删除前源码。相同 repo+SHA 幂等采集，后台与设计稿新增「仓库证据」页面。
## 2026-09-14（P08 执行契约补尾）

- **需求执行契约与自动执行门禁**：执行仓库、允许改动路径、结构化验收命令、最长时长、尝试次数和分单位预算进入需求内容修订；修改任一项都会使旧批准失效。详情页可编辑契约、审批前可查看完整契约，并展示未批准、状态不对、依赖缺失/未完成和自动执行未启用等阻塞原因。依赖按同仓库导入链接解析，规划需求只有 `done` 才算就绪，能力基线仍视为未验收。本批不启动执行器、自动合并或部署。
