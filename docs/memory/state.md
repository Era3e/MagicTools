...
- **D-09 意图路由在线学习落地（2026-08-28，PR #43 draft 已创建待 CI）**：三层闭环——① few-shot 在线注入（IntentService 从纠错样本均衡采样构造示例注入 system prompt，每意图 3 条/总数 12 封顶，60s TTL 缓存，纠错落库即清缓存即时生效）；② 评估闭环（EvaluationService：混淆矩阵 + 回放评估命中率，`GET /intent-logs/evaluation[/replay]`）；③ 数据集导出（OpenAI 兼容 JSONL，`GET /intent-logs/export` + 前端 Blob 下载）。前端 IntentLogPage 新增「路由评估」卡片（回放按钮/混淆对表格/导出数据集）。真 LoRA 微调继续延期（导出格式已就绪）。同批含 D-06/D-08/D-12/D-13（分支 feat-deferred-batch-d06-d08-d09-d12-d13，8 提交）。验证：assistant-server 81/81（含 3 条新 e2e）、assistant-web 13/13、pnpm lint 0 err、test:affected 33/33；
