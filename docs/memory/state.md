...
- **P06开发进行中（2026-09-12，feat-gateway-P06-auth，基于761152b）**：spec docs/superpowers/specs/2026-09-12-gateway-auth-design.md已立。TDD完成users/session/auth三模块（gateway 44/44）+e2e gateway-auth.spec 4/4（独立3999认证实例真实浏览器登录/登出/错误文案/401 JSON）。assistant action/trouble接入GATEWAY_ASSISTANT_SERVICE_TOKEN回落。**两个新坑入库**：①GATEWAY_USERS的user:hash:role解析与scrypt哈希内冒号冲突——哈希格式改`scrypt$盐$hex`（$分隔）；②auth中间件先于body解析挂载导致登录POST `req.body undefined`崩溃进程（e2e表现为chrome-error://chromewebdata/，实为网关进程崩溃）——中间件内部自解析urlencoded body（16KB上限，拒绝express深路径import因TS2307无类型声明）。gateway index支持PORT_OVERRIDE供e2e独立实例。@mt/db dist陈旧（9/9）致assistant lint假红，rebuild即绿——stale dist陷阱第N次。沉淀层文档已同步（features/gateway-auth.md、CODE_WIKI §5.1、coverage P04行更新+P06行、CHANGELOG、changeset、env模板6新变量）；docs:lint 63文件0错误、design:check 126 PASS。qa:gate运行中，全绿后独立验收→PR→CI→合并。

- **P04完整本地验收已通过，准备候选提交（2026-09-12）**：新冻结qa53bfb05c458840171976eea1全7阶段通过，infra353/353、真实DB31文件136/136零跳过，Node20 smoke17。独立正式run594bc2f98a44f636的11场景与清理全部通过，独立回读18/18；原2容器/4网络/8卷/109镜像及5432/55432/55433/55101和17预览监听保持，测试资源和私有文件均清除。平台恢复样本61793ms，active cron5次runs且独立receiver始终1次正向请求。源码fingerprint0ec68b78cb61dc9c1eea1f11daf0cc56b83beb4979869bef8d49f58286b59f9d与qa一致，HEAD仍09cb670+本轮WIP。首轮5f失败及cleanup-supplement保留，不改旧报告。收尾文档在验收后补记，不冒称后续提交指纹；待新候选CI精确核验后合PR74，再继续P06及全部剩余规划。
