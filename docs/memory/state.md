...
- **项目阶段**：Phase 1 · Applicant 试点。设计草案（spec）与实施计划（9 任务）均已完成并入库，等待用户确认 A1~A4 四项默认假设后即可开工。
- **下一步**：用户确认 A1~A4 → spec 定稿 → 重建 dev 分支 → 按计划执行 T1~T9 → CI 全绿 → PR 合并 main；随后需求主线（Investigator→Assessor→Manager）。
