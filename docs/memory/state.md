...
- **P06合并闭环与CI修复实录（2026-09-12，PR #75 → 6bdfcb5）**：PR #75 首轮 CI quality 43秒即死——根因是D5返工时 auth.test.ts 两处用例遗留未使用的 `const hash`（eslint no-unused-vars 首阶段挂掉；教训：**修复后必须重跑完整 qa:gate，不能依赖修复前的门禁结果**）。本地合并 origin/main（815ede3 版本bump+CHANGELOG，零冲突）后复现修复，途中处置 gatherer-web build `ENOTEMPTY`（29个残留 node 进程锁 dist——**会话收尾必须杀净服务进程**）。qa:gate 7阶段全绿（回执50d80a5b925fff9008a976eb）后推送 2e48974，CI run 34689112392 三段全绿（quality/smoke/e2e，images 按PR规则skip），squash 合并 **6bdfcb5**，本地 main 已同步，分支已删。changeset-release/main 已被 bot 强推，发布 PR 待 bot 开立后走三件套（补勾选+close/reopen+CI绿后squash）。**P06 内容**：GATEWAY_USERS 会话登录（scrypt$salt$hex+HMAC Cookie+限流）/GATEWAY_SERVICE_TOKENS 服务身份/GATEWAY_TOKEN 共享三通道；两轮 0 bug loop 验收（首轮 D1 高危 DoS 等五缺陷全修复）。
...
- **P06开发完成待PR（2026-09-12，feat-gateway-P06-auth，6306e32+39b26ee）**：spec/docs/沉淀层全同步。**0 bug loop 两轮**：首轮独立验收不通过（D1高危未认证远程DoS——decodeURIComponent("%ZZ")在req.on("end")回调内抛URIError击穿进程、D2 x-gateway-user用res.setHeader只写网关响应头未透传下游且测试读响应头假绿、D3文档失实、D4/D5低危）→返工修复（end回调内try/catch+reject映射400；setGatewayIdentity改req.headers注入+入口delete伪造头；assistant双分支token用例；appOfPath单正则简化）→二轮复验全部通过（gateway 47/47、assistant 91/91、e2e 4/4、lint双零、D1真实HTTP探针400+存活）。**本地门禁**：qa:gate 7阶段全过（回执01ee197bad0d694720e0cb8f）+smoke 17/17+e2e全量89条。**四个环境坑入库**：①pull后必须pnpm install——P01/P02给根加的vitest/typescript devDeps未装导致infra/testing下setup文件vi.mock失效（setup与测试解析到不同vitest实例），unit-boundary假红；②本机Node已升25.2.1，下载便携Node 20.20.2（d:\node20-runtime）前置PATH跑门禁（P04验收同版本）；③qa:gate database阶段需显式MT_TEST_DATABASE_URL指向postgres管理库（…/postgres非业务库）；④本地manager演示库旧schema撞迁移约束——start-services.mjs给manager-server加独立manager_e2e库（与CI口径一致），manager-approval e2e需MANAGER_APPROVAL_TOKEN（已写入.env并重启manager-server）。backup-wrappers两用例环境敏感失败（8.3短名TEMP vs长名）——assertCwd用realpathSync.native归一（argv断言仍严格）。P06全程未破坏P04/P05验证链（GATEWAY_TOKEN兼容模式）。\n\n- **P06推送实录与PR #75（2026-09-12）**：git push 写通道再度长时间挂死（HTTP/1.1+LOW_SPEED+重试均无效）→按记忆降级链走 MCP push_files 分批推送（Gateway/assistant 代码文件+changeset+env模板+features/spec/CHANGELOG/coverage逐一推齐），期间两个临时暂存文件已删净；**约2小时后代理间歇恢复，git push -f 一次性覆盖成功**——本地完整提交链 2c01d21 同步远端（42文件缺失0，含此前受128KB读取限制的CODE_WIKI/state.md）。PR #75 已开（body 双勾选+两轮0bug实录+验证边界），CI 在 head 2c01d21 上运行（quality 进行中）。**推送策略沉淀：大文档走 MCP push_files 受单次调用体积限制，本地网络恢复前先开 PR 让 CI 跑代码层，文档补推与 CI 并行——CI 任何红灯都在 head 变更后重跑，无浪费**。待CI三段绿→按授权squash合并→同步本地main。

- **P04合并闭环与本会话接续（2026-09-12）**：本会话按用户授权（无需再逐项确认）核验PR #74验收记录与CI三段绿（quality/smoke/e2e全过，images skipped为无registry预期）后转正并squash合并**761152b**。CI `pull_request` 无 types 限制（默认 opened/synchronize/reopened），ready 转换不触发重跑——head SHA 5986667 上的绿灯即最终核验。PR #73（release）三件套完成：bot已消费P04 changeset更新head至c1bc361，补body两勾选+close/reopen触发CI（run 34678664848），quality/smoke/e2e三段全绿后squash合并**815ede3**。本地main已同步761152b；P系列清单不在仓库/GitHub Issues/本地manager库（旧schema无P数据），以文档证据链（restored-deployment.md:96、runtime-images.md:19、P04 plan:40三处一致）确认P06=生产用户登录与服务权限。

- **P04故障镜像清理闭环与再冻结（2026-09-12）**：独立扩展发现仅在RepoDigests出现的外来引用仍需保护，已补RED→GREEN；最终独立26/26、开发10/10通过。修后helper729efc35a47e588ac0990b23ed38e99bbb2812f5bd36ff37560eb959ac4e57d9真实最小回归仅删除原run5f865210d87e0f73记录的镜像ID，其它镜像集合保持，原失败summary字节不变，cleanup-supplement已成功。因此该轮测试资源现在全部清理完成，但原整轮仍不改绿。修复期间qa78c89059fb7c577b775ab2e7因fingerprint变化最终失败，不能采用；源码再次冻结，准备新完整qa与独立全流程。P06只读调研已在work/P06-权限边界现状与实施建议.md完成，尚未开发。

- **P04正式实机11场景通过但镜像清理失败（2026-09-12）**：qa6ee46c804931dc0a2dcbcc74全7阶段通过，infra352/DB31文件136零跳过，Node20源码smoke17；被测WIP指纹31aed96b9f6fc888c8384f99ee4c73f03a2bb6b5e5280d1563e8bb6d4f89135f。独立run5f865210d87e0f73完成全部11真实场景（包括cron外联、拉取/迁移失败及同名替换），但containerd把本轮digest放入RepoTags导致故障镜像清理误拒，最终仍exit1/failure。原2容器/4网络/8卷和主机监听/预览保持，私有文件已清、源码未变；约271MB本轮故障镜像待补清。根已以真实元数据建立RED并修复，只允许已记录且同repo/实际RepoDigests验证的发布引用，外来tag/digest仍拒；入口记录reference，开发10/10通过，独立最小清理复验及新完整实机/qa/最终CI继续。旧失败报告不得改绿；PR74未提交新WIP、未合并。

- **P04交接回执独立复验关闭（2026-09-12）**：独立发现首次验证时间不严格/早于恢复、配置与回执共同漏业务库、恢复名撞应用DNS三类误接受，已按RED→GREEN修复；原10组加时间/时区/闰日/34名称/集合顺序等补验共14/14，证据work/recovery-ssh-receipt-review/20260912。repo联合部署/SSH58/58、最新infra348/348零跳过；操作文档认证入口缺口已关闭，独立核对4份真实成功回执及7个链接。仍未提交：正式全平台演练入口与CI接入继续整理，当前没有新完整qa/smoke/最终候选CI，不提前合并P04。

- **P04交接回执与故障回归补齐（2026-09-12，未提交）**：新增recovery-receipt.mjs，SSH正常/回退在17ready之外核对真实DB身份、claim、首次目录证明及12条连接，仅复制验证过的公开字段。修正初始测试夹具缺少平台/镜像ID后得到真实RED，再实现GREEN；与旧SSH/部署联合55/55、零跳过，新增首次检查中断后claim重试、config/pull/up/verify失败保留current、拒绝换库/v1/换claim及17ready后身份替换。完整用户操作章节docs/features/restored-deployment.md、CODE_WIKI/coverage/CHANGELOG/changeset已同步，文档lint4文件无错误；MJS仍按仓库配置被ESLint忽略，不能称额外ESLint覆盖。独立测试与文档复核进行中，正式Docker演练入口正在整理。最终全仓qa/smoke、实机全流程和新候选CI尚未执行，PR74保持草稿；既有09cb CI成功只对应旧传输阶段。

- **P04交接WIP阶段复验（2026-09-12）**：最新基础设施回归328/328、零跳过；资源/受控ingress/attachment独立联合65/65通过，原孤立前缀资源、网关任意外网及损坏claim缺陷均关闭；handoff/部署独立14/14关闭实时密文变化及输出污染备份目录问题。仅为当前未提交WIP阶段验收，仍需SSH v2回执、剩余实际故障/业务外联场景、正式验证入口与干净完整重跑、全仓qa/smoke及最终CI；不能据此完成或合并P04。

- **P04恢复部署交接WIP（2026-09-12，基于09cb670，未提交）**：新增deployment-config/2恢复绑定、backup:handoff、12条主/跨库连接改写、真实资源身份和原子claim、reserved→initial-verified持久阶段，以及P05 v2仅管理17应用的部署/回退接入。v1配置版本保持兼容；旧A制品精确默认连接仅在派生配置迁为私有env引用。真实内部网络使宿主网关不可达后，改为gateway-only受控ingress，业务/web/PG仍在内部网，任意outside拒绝。资源85条开发回归及配置/连接独立16项、handoff/部署独立14项、attachment独立9项通过；最新资源/ingress独立复验进行中。真实核心210cae80d5e11edd完成handoff/claim/首次阶段及清理；全平台0ff243cfbca71e1f实测17ready、12应用PG连接、Manager API读写隔离、A→B→A与坏密码失败/回退。原run在最后无序Mounts数组比较误报；16次只读检查证明仅顺序不同，preservation-supplement补核源/私有env并清理通过，保留原始失败报告，不冒称整个原run全绿。正式干净全流程、其它故障与外联场景、SSH v2回执校验、文档收尾及最终qa/CI仍待完成，P04仍不合并。09cb阶段CI34664790384已全部成功并独立核验，不能覆盖这些新WIP。
