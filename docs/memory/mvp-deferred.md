...
| **D-07** ✅ PR #36 | 落地降级 | Investigator **Cron 自动调度 Bitable 拉取** | investigator-spec.md §3.2 定时拉取；gatherer-spec 已有 node-cron 先例 | MVP 仅实现管理员 SurveyDetail 页「立即同步」手动按钮；scheduler 骨架代码未挂载 | —（已落地：node-cron scheduler + GET /api/health + 状态 API） | — | — |
...
| **D-10** ✅ PR #35 | 落地降级 | Gateway **统一健康监控仪表盘** | CODE_WIKI §5 Gateway 有 health 聚合，缺 UI | MVP `/health` 只返回 JSON，无可视化 UI；Assistant trouble_shooting 是文字版替代 | —（已落地：/api/health 聚合 + /status 仪表盘 Chart.js） | — | — |
| **D-11** ✅ PR #35 | 流程纪律 | 0 bug loop **开发/测试双智能体对抗验收记录** | AGENTS.md §硬性约定 6；state.md 已知问题 9 | MVP 仅文档声明「0 bug loop」，但当前环境 subagent_fork 不可用（state.md 已知问题 8），无测试 agent 独立验收记录产物 | —（已落地：PR 模板加 UI Checklist + 0 bug loop 验收记录段） | — | — |
...
| **D-16** ✅ PR #39 | 落地降级 | Designer 前台页 **「组件馆藏」导航入口** | e2e designer.spec 导航跳转用例；2026-08-27 显式 skip 发现 | 前台 UserShell 导航只有「定制生成」，无通往 /admin/components 组件馆藏的入口 | —（已落地：UserShell nav 加组件馆藏项） | — | — |
| **D-17** ✅ 已落地 | 落地降级 | Assistant **「意图日志」导航入口** | e2e assistant.spec 导航跳转用例；2026-08-27 显式 skip 发现 | 反馈页（/admin/feedback）侧栏无「意图日志」菜单，仅能手动输 URL /admin/intent-logs 到达 | —（已落地：AdminShell nav 加意图日志项） | — | — |
...
- **已完成**：7 项 ✅ — D-05（Scholar 图谱 G6 交互 PR #38）、D-07（Investigator Cron PR #36）、D-14（Manager 燃尽图 PR #40）（Investigator Cron PR #36）、D-10（Gateway 健康仪表盘 PR #35）、D-11（0 bug loop PR #35）、D-16（Designer 前台 nav PR #39）、D-17（Assistant 意图日志 nav 已落地）
- **剩余**：12 项（原 18 项，减去已完成 7 项）
- 剩余按来源：**功能裁剪** 4 项（D-01/02/09） · **落地降级** 6 项（D-03/06/08/12/13/18） · **规划遗漏** 2 项（D-04/14/15）
- 剩余按优先级：P1 7 项 · P2 3 项 · P3 1 项
- **下迭代建议兑现**：D-14（Manager 燃尽图，patterns/TimelineBurndown 新增，前端改动量可控）、D-18（linux 视觉基线，CI 里一次性生成入库）
- D-03（Webhook）/ D-04（一键 PR）需要 GitHub API / Webhook 基础设施，适合打包处理
- D-06（Obsidian 冲突合并）工作量大，适合中期
- D-13（移动端响应式）是全平台系统性改造，适合 v1.1 集中迭代
