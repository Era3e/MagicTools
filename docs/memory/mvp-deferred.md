...
| **D-04** | ~~规划遗漏~~ ✅ **已兑现** | Designer 组件「**一键 PR 到 @mt/ui**」 | designer-spec.md 未覆盖；coverage-matrix D9 | 组件审核入库只进 designer 本地 ComponentRepo，不自动提 PR 到主仓 packages/ui | ✅ 2026-08-29 兑现（PR #42）：GitHubClient 三步流（createBranch/createFile/createPr，PAT + GITHUB_STUB 桩）+ publish.service（4 单测）+ POST /components/:id/publish + ComponentList「一键 PR」按钮/结果 Modal | ✅ P1 已完成 | — |
