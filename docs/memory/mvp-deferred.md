...
| **D-07** | ~~落地降级~~ ✅ **已兑现** | Investigator **Cron 自动调度 Bitable 拉取** | investigator-spec.md §3.2 定时拉取；gatherer-spec 已有 node-cron 先例 | MVP 仅实现管理员 SurveyDetail 页「立即同步」手动按钮；scheduler 骨架代码未挂载 | ✅ 2026-08-28 兑现：node-cron 自动调度（scheduler.ts + cron 列 + startScheduler + meta/scheduler-status API） | ✅ 已完成 | P0（低工作量） | — |
...
| **D-09** | 功能裁剪 | Assistant **意图路由自训练微调（模型路由在线学习）** | assistant-intents-design.md §6 未来工作；state.md 纠错回填已做 | ~~MVP 纠错回填只改路由规则~~ **2026-08-28 已兑现在线学习层**：纠错样本自动构造 few-shot 注入分类提示词（每意图 3 条/总数 12 封顶、60s TTL 缓存、纠错落库即清缓存即时生效）；新增评估闭环（混淆矩阵 `GET /intent-logs/evaluation`、回放命中率 `…/replay`）与 OpenAI 兼容 JSONL 数据集导出（`GET /intent-logs/export`，IntentLogPage 路由评估卡片可视化）。**真 LoRA 微调仍延期**（导出格式已就绪，积累样本后可直送训练平台） | few-shot 注入已上线；LoRA 待样本量 ≥ 数百条 + 智谱 Key 恢复 | P3 | few-shot 层已落地（2026-08-28）；LoRA 层触发条件：intent_logs 纠错样本 ≥ 500 条 + LLM 供应商支持微调 API |
...
| **D-11** | ~~流程纪律~~ ✅ **已兑现** | 0 bug loop **开发/测试双智能体对抗验收记录** | AGENTS.md §硬性约定 6；state.md 已知问题 9 | MVP 仅文档声明「0 bug loop」，但当前环境 subagent_fork 不可用（state.md 已知问题 8），无测试 agent 独立验收记录产物 | ✅ 2026-08-28 兑现：CI quality job 新增条件 step（仅 PR 事件）检测复选框勾选状态；PR 模板已含复选框 | ✅ 已完成 | P0 | — |
...
| **D-16** | ~~落地降级~~ ✅ **已兑现** | Designer 前台页 **「组件馆藏」导航入口** | e2e designer.spec 导航跳转用例；2026-08-27 显式 skip 发现 | 前台 UserShell 导航只有「定制生成」，无通往 /admin/components 组件馆藏的入口（仅页脚「管理后台 →」间接可达） | ✅ 2026-08-28 兑现：USER_NAV 加「组件馆藏」+ Route 改直接渲染 ComponentList | ✅ 已完成 | P2（低工作量） | — |
| **D-17** | ~~落地降级~~ ✅ **已确认已修复** | Assistant **「意图日志」导航入口** | e2e assistant.spec 导航跳转用例；2026-08-27 显式 skip 发现 | 反馈页（/admin/feedback）侧栏无「意图日志」菜单，仅能手动输 URL /admin/intent-logs 到达 | ✅ 已确认：ADMIN_NAV 已有「意图日志」菜单 + /admin/intent-logs Route + IntentLogPage，2026-08-27 后代码已补齐 | ✅ 已完成 | P2（低工作量） | — |
...
- 总数：18 项
- **已兑现 4 项**：D-07（Cron 拉取）、D-11（0 bug loop CI 检测）、D-16（Designer 导航入口）、D-17（Assistant 导航入口，确认已修复）
- 按来源：**功能裁剪（规划时决定不做 MVP）** 4 项（D-01/02/09/09） · **落地降级（有骨架但缺完整实现）** 8 项（D-03/05/06/08/10/12/13/18） · **规划遗漏（spec 没写但理应存在）** 3 项（D-04/14/15） · **流程纪律** 0 项（D-11 已兑现） · 已兑现 4 项
- 按优先级：P0 **全部兑现**（D-07/D-11）· P1 10 项 · P2 3 项（D-15 待定、D-16/D-17 已兑现）· P3 1 项
- **下一迭代建议**：D-03（Manager PR Webhook，部署前提）、D-18（linux 视觉基线，CI 全绿最后一环）、D-05（Scholar 图谱力导向图）— 三项合计工时 1-2 天。
