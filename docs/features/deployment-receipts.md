...
恢复现有备份时使用`backup:handoff`生成v2公开配置，新项目仅启动17应用并验证独立恢复PG。不要手动修改普通v1配置中的数据库地址代替交接；准备、12条连接、受控入口网络、回执和失败处置见[从备份恢复应用](restored-deployment.md)。v1仍保留自建数据库的原运行契约。
...
本批代码提供本机和SSH入口。cc03f42→5355139的真实Docker升级、故障恢复和回退已在独立测试项目通过，独立验收另行核对八库数据、秘密文件和数据库实例均保持；证据见 [验收记录](../validation/2026-09-12-runtime-images.md)。SSH传输、退出码及回读逻辑使用可控适配器回归，实际生产SSH主机需在提供地址和凭证后验证。这些结果不代表生产环境已经上线或真实模型质量已经提升。
