...
- **E1**：process_execution 动作 = 创建需求（写 Manager）+ 触发采集（调 Gatherer）
- **E2**：trouble_shooting = 实时探测各服务 /health（经网关）+ LLM 结合探测结果生成建议
- **E3**：complaint_feedback = feedback 表落库 + 列表 API + Web 反馈页查看/删除
