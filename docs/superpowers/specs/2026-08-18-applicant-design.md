...
- 文档类型：子项目设计文档（spec）
- 创建日期：2026-08-18
- 状态：🟡 草案（待确认 4 项默认假设 A1~A4，见第 6 节）
...
## 6. 待确认的默认假设（回来只需回复确认或修正）

- **A1**（原 Q1）：图片识别主要支持「招聘平台 JD 截图 + 聊天/内推截图」，线下物料拍照后置
- **A2**（原 Q2）：先做 Applicant，需求主线随后
- **A3**（原 Q3）：JD 解析只做「粘贴文本」；URL 抓取因反爬风险放入 Backlog
- **A4**（原 Q4）：简历主路径 = ClawCV API；手动粘贴文本作为降级与补充；PDF 本地上传解析放入 Backlog
