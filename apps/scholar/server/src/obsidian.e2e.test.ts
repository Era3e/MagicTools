...
describe("obsidian", () => {
  ...
  it("未配置 vault 路径时同步返回 400", async (ctx) => {
    if (!available) { ctx.skip(); return; }
    const res = await request(app.getHttpServer()).post("/api/scholar/sync/obsidian");
    expect(res.status).toBe(400);
  });

  it("PATCH /api/scholar/settings 配置并回读 vault 路径", async (ctx) => {
    if (!available) { ctx.skip(); return; }
  ...
  it("POST /api/scholar/sync/obsidian 扫描导入并跳过模板/附件目录", async (ctx) => {
    if (!available) { ctx.skip(); return; }
  ...
  it("GET /api/scholar/meta/embedding-status 返回桩模式状态", async (ctx) => {
    if (!available) { ctx.skip(); return; }
