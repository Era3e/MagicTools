    ...
    >
      <Table<Survey>
        ...
        columns={[
          ...
          { title: "状态", dataIndex: "status", width: 90, render: (v: string) => <MtStatusTag tone={v === "active" ? "success" : "neutral"} mono>{v}</MtStatusTag> },
          { title: "最近同步", dataIndex: "lastSyncedAt", width: 180, render: (v: string | null) => (v ? <span style={{ fontFamily: tokens.font.mono, fontSize: 12 }}>{new Date(v).toLocaleString()}</span> : "-") },
          {
            title: "操作",
            width: 100,
